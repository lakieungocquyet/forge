...
while read -r sample; do
    ...
    GVCF_COMBINE_FLAG+=" -V ${OUTPUT_DIR_PATH}/${sample_id}/${sample_id}.g.vcf"

    logger INFO $RUNTIME_LOG_FILE_PATH "Process sample ${green_color}$sample_id${reset}"
    ...
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Map and align ${green_color}$sample_id${reset} reads to reference genome"

    logger INFO $RUNTIME_LOG_FILE_PATH "Convert ${green_color}$sample_id${reset} mapping and alignment result from SAM format to BAM format"
    
    run_command_with_monitoring "
        /usr/bin/time -v -a -o '${RUNTIME_LOG_FILE_PATH}' \
            samtools view -@ ${THREADS} -Sb ${OUTPUT_DIR_PATH}/${sample_id}/${sample_id}.sam \
                > ${OUTPUT_DIR_PATH}/${sample_id}/${sample_id}.bam
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Convert ${green_color}$sample_id${reset} mapping and alignment result from SAM format to BAM format"

    logger INFO $RUNTIME_LOG_FILE_PATH "Sort all records in ${green_color}$sample_id${reset} BAM file by genomic coordinates"
    run_command_with_monitoring "
        /usr/bin/time -v -a -o '${RUNTIME_LOG_FILE_PATH}' \
            samtools sort -@ ${THREADS} -o ${OUTPUT_DIR_PATH}/${sample_id}/${sample_id}.sorted.bam \
                ${OUTPUT_DIR_PATH}/${sample_id}/${sample_id}.bam
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Sort all records in ${green_color}$sample_id${reset} BAM file by genomic coordinates"
    ...
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Mark duplicate reads in ${green_color}$sample_id${reset} BAM file"
    ...
    if [ ${#BQSR_FLAGS[@]} -eq 0 ]; then
        ...
        " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Apply BQSR to ${green_color}$sample_id${reset}"
    fi
    ...
    run_command_with_monitoring "
        /usr/bin/time -v -a -o '${RUNTIME_LOG_FILE_PATH}' \
            gatk HaplotypeCaller \
                ...
                --verbosity INFO
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Call variants (GVCF) for ${green_color}$sample_id${reset}"
...
" ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Combining GVCF files for samples: ${green_color}${sample_ids}${reset}"

logger INFO $RUNTIME_LOG_FILE_PATH "Genotyping combined GVCF for samples: ${green_color}${sample_ids}${reset}"
run_command_with_monitoring "
    /usr/bin/time -v -a -o '${RUNTIME_LOG_FILE_PATH}' \
        gatk GenotypeGVCFs \
            -R ${REFERENCE_GENOME_FILE_PATH} \
            ...
            --read-validation-stringency SILENT \
            --verbosity INFO
" ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Genotyping combined GVCF for samples: ${green_color}${sample_ids}${reset}"

logger INFO $RUNTIME_LOG_FILE_PATH "Filtering variants for samples: ${green_color}${sample_ids}${reset}"
run_command_with_monitoring "
    /usr/bin/time -v -a -o '${RUNTIME_LOG_FILE_PATH}' \
...
" ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Filtering variants for samples: ${green_color}${sample_ids}${reset}"
...
" ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Normalizing combined VCF for samples: ${green_color}${sample_ids}${reset}"
...
" ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Annotate variants with genomic information for samples: ${green_color}${sample_ids}${reset}"
...
" ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Annotate variants with variant type for samples: ${green_color}${sample_ids}${reset}"
...
if [ -f "$CLINVAR_FILE_PATH" ]; then
    ...
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Annotate variants with ClinVar database for samples: ${green_color}${sample_ids}${reset}"
else
    logger WARNING $RUNTIME_LOG_FILE_PATH "Skip clinvar annotation (file not found)"
...
if [ -f "$PHASE3_1000G_V4_20130502_FILE_PATH" ]; then
    ...
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Annotate variants with 1000g phase3 database for samples: ${green_color}${sample_ids}${reset}"
else
    logger WARNING $RUNTIME_LOG_FILE_PATH "Skip 1000g phase3 annotation (file not found)"
...
if [ -f "$ESP6500SI_V2_SSA137_FILE_PATH" ]; then
    ...
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Annotate variants with ESP6500 database for samples: ${green_color}${sample_ids}${reset}"
else
    logger WARNING $RUNTIME_LOG_FILE_PATH "Skip esp6500 annotation (file not found)"
...
if [ -f "$DBSNP_138_FILE_PATH" ]; then
    ...
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Annotate variants with dbSNP 138 database for samples: ${green_color}${sample_ids}${reset}"
else
    logger WARNING $RUNTIME_LOG_FILE_PATH "Skip dbSNP138 annotation (file not found)"
...
if [ -f "$DBNSFP_FILE_PATH" ]; then
    ...
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Annotate variants with dbNSFP database for samples: ${green_color}${sample_ids}${reset}"
else
    logger WARNING $RUNTIME_LOG_FILE_PATH "Skip dbNSFP annotation (file not found)"
...
while read -r sample; do
    ...
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Extract variants for ${green_color}$sample_id${reset}"
    ...
    " ${MONITORING_LOG_FILE_PATH} ${MONITORING_STREAM_LOG_FILE_PATH} "Generate SNP and Indel variants XLSX report for ${green_color}$sample_id${reset}"
